...
conditions: >
  ...
  possible means available to power). When power is bound to violence as its
  primary mode of operation (overt dictatorship, military rule, occupation),
  the binding amplifies violence's reach across the field but degrades
  power's reserves. Quality is low: counter-pattern formation (resistance,
  fear-based compliance that erodes under any weakening of enforcement,
  depletion of biological reserves of the subject population) generates
  field-degradation that the binding cannot offset. Strength is variable:
  some power is bound only weakly to violence (legitimate state holding
  violence as backstop) — this is the substrate-dependence case, not the
  binding case; the binding-relation is recorded when power's mode of
  operation IS violence. Durability is contingent and degrading: the binding
  consumes power's reserves faster than violence-based exercise can replenish
  them. Sustained tight-binding-to-violence is rare without collapse.
status: Draft
version: 0.3
```
...
conditions: >
  ...
  channel through which power exercises. The binding is structurally low-
  quality: coercion's internally-consonant-externally-dissonant character
  means the binding amplifies coercion's reach but degrades power's reserves
  (counter-pattern formation reduces voluntary compliance, depleting the
  legitimacy and trust reserves power requires for sustained operation).
  Strength is variable: power can be loosely bound to coercion (coercion as
  one tool used sparingly) or tightly bound (coercion as primary mode of
  operation, "coercive power"). Durability is contingent and degrading:
  power tightly bound to coercion erodes its own substrate over time; the
  binding is durable only as long as power's reserves can supply the
  contingent costs coercion requires. Emerges when reciprocity-based binding
...
conditions: >
  ...
  power but supplies no field-supportive amplification — power amplifying
  itself without amplifying any value-pattern the field is composed of.
  Quality is low: there is no field-supportive pattern being amplified.
  Strength is variable: some power can self-perpetuate weakly while still
  maintaining other bindings; some power is bound primarily to its own
  perpetuation with other bindings degraded. Durability has a specific
  character: the binding produces the internally-consonant-externally-
  dissonant signature in the field — the binding persists internally (power
  continues to operate as power, sustaining itself) but produces field-
  degradation through counter-pattern formation, legitimacy erosion, and
  substrate exhaustion. The configuration is durable for years to decades,
  rarely longer.
status: Draft
version: 0.3
```
